...
class JournalDaoImpl(client: DynamoDBTaskClientV2,
  ...
  private def updateJournalRows(journalRows: Seq[JournalRow]): Task[Long] = {
    def loop(values: Map[String, Seq[WriteRequest]]): Task[Long] = {
      if (values.isEmpty)
        Task.pure(0L)
      else
        client
          .batchWriteItem(values).flatMap { result =>
            if (result.isSuccessful) {
              result.unprocessedItems
                .map { ui =>
                  loop(ui)
                }.getOrElse(0)
              Task.pure(journalRows.size)
            } else
              Task.raiseError(new Exception())
          }
    }
    loop(
      convertToBatchWriteItem(journalRows)
    )
  }

  private def convertToBatchWriteItem(
      values: Seq[JournalRow]
  ): Map[String, Seq[WriteRequest]] = {
    Map(
      tableName -> values.map { x =>
  ...
  private def getHighestMarkedSequenceNr(persistenceId: String): Task[Option[Long]] = {
    client
      .query(
        QueryRequest()
          .withTableName(Some(tableName))
          .withKeyConditionExpression(Some("#pid = :pid"))
          .withFilterExpression(Some("#d = :flg"))
          .withExpressionAttributeNames(
            Some(Map("#pid" -> PersistenceIdColumnName, "#d" -> DeletedColumnName))
          )
          .withExpressionAttributeValues(
            Some(
              Map(
                ":pid" -> AttributeValue().withString(Some(persistenceId)),
                ":flg" -> AttributeValue().withBool(Some(true))
              )
            )
          )
      ...
      .map(_.map(_.sequenceNumber).headOption)
  }

  private def deleteBy(persistenceId: String, sequenceNr: Seq[Long]): Task[Unit] = {
    if (sequenceNr.isEmpty)
      Task.pure(())
    else
      client
        .batchWriteItem(Map(tableName -> sequenceNr.map { snr =>
              ...
              DeleteRequest().withKey(
                Some(
                  Map(
                    ...
                    SequenceNrColumnName    -> AttributeValue().withNumber(Some(snr.toString))
                  )
                )
              )
            )
